...
Bitmap *read_header() {
    int byte_location = 0;
    int error;
    //Read in BMP_HEADER_SIZE_OFFSET bytes
    unsigned char data[BMP_HEADER_SIZE_OFFSET];

    error = fread(data, sizeof(char), BMP_HEADER_SIZE_OFFSET, stdin);

    if (error!=BMP_HEADER_SIZE_OFFSET){
        fprintf(stderr, "Could not read metadata\n");
        exit(1);
    }
    byte_location = BMP_HEADER_SIZE_OFFSET;

    //Getting the header size
    int header_size;
    error = fread(&header_size, sizeof(int), 1, stdin);

    if (error!=1){
        fprintf(stderr, "Could not read header size\n");
        exit(1);
    }
    byte_location +=4;

    Bitmap *image_header = malloc(sizeof(Bitmap));

    //Setting size of header
    image_header->headerSize = header_size;

    // Storing header data in array
    unsigned char *header_data = malloc(sizeof(char)* (header_size));

    // Coping already read data
    memcpy(header_data, data, BMP_HEADER_SIZE_OFFSET);
    memcpy(header_data+BMP_HEADER_SIZE_OFFSET, &header_size, sizeof(int));

    // Adding remaining data until BMP_WIDTH_OFFSET
    unsigned char data2[BMP_WIDTH_OFFSET-byte_location];
    error = fread(data2, sizeof(char), BMP_WIDTH_OFFSET-byte_location, stdin);
    if (error!=BMP_WIDTH_OFFSET-byte_location){
        fprintf(stderr, "Could not read metadata\n");
        exit(1);
    }
    memcpy(header_data+BMP_HEADER_SIZE_OFFSET+4, data2, BMP_WIDTH_OFFSET-byte_location);

    byte_location = BMP_WIDTH_OFFSET;
    // Read in width and height
    int width;
    int height;
    error = fread(&width, sizeof(int), 1, stdin);
    if (error!=1){
    ...
    memcpy(header_data+BMP_WIDTH_OFFSET, &width, sizeof(int));

    error = fread(&height, sizeof(int), 1, stdin);
    if (error!=1){
    ...
    memcpy(header_data+BMP_HEIGHT_OFFSET, &height, sizeof(int));

    // Read remaining metadata
    unsigned char data3[(header_size)-(byte_location)];
    error = fread(data3, sizeof(char), (header_size)-(byte_location), stdin);
    if (error!=(header_size)-(byte_location)){
        fprintf(stderr, "Could not read metadata\n");
        exit(1);
    }
    memcpy(&header_data[BMP_HEIGHT_OFFSET+4], data3, (header_size)-(byte_location));
    image_header->header = header_data;

    image_header->scale_factor =1;
...
void scale(Bitmap *bmp, int scale_factor) {
    ...
    // width
    // file size
    // contents in header with new values

    int new_width = bmp->width * scale_factor;
    int new_height = bmp->height * scale_factor;
    int new_file_size = sizeof(Pixel) * new_height * new_width + bmp->headerSize;

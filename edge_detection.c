...
void edge_detection_filter(Bitmap *bmp) {
    ...
    for (int row = 0; row<bmp->height; row++){
        for (int column = 0; column < bmp->width; column++){
            ...
            if (column ==0){
                result = apply_edge_detection_kernel(row1, row2, row3);
            }else if (column == bmp->width-1){
                result = apply_edge_detection_kernel(row1+((column-2)), row2+((column-2)), row3+((column-2)));
            }else{
                result = apply_edge_detection_kernel(row1+((column-1)), row2+((column-1)), row3+((column-1)));
            }
